...
a = 0 # used for sum 
count = 1 #used for student number
print("----------------------")
print("Student Averages:")
print("----------------------")

#Find avg of row (Studennt) in array gradebook
for i in range(len(gradebook)):
    ...
    #calc average
    avg = a / len(gradebook[i])
    # Print result
    print("Student", count, ":", round(avg, 2))
    count+=1 # go to next student
    a = 0 # reset sum
...
count = 0 #used for assignment number
list = [] #all assignment grades wil go into a list

#find assignment grade average for all gradebook array columns
for i in range(len(gradebook[i])):
    for j in range(len(gradebook)):

        #add each students row int to a list
        list.append(gradebook[j][count])
        x = sum(list) / len(list)

    count  = count + 1 # move to next assignent
    list = [] #reset lsit

    #print result
    print("Assignment:" , count , round(x, 2))
